...
public class NPC_Controller : MonoBehaviour
{
    ...
    private IEnumerator EatFood()
    {
        ...
        if (waveManager.waveNumber <= 2)
        {
             foodParent = food1Slot.transform;
        }
        else
        {
            foodParent = food2Slot.transform;
        }
        ...
        while (foodParent.childCount > 0)
        {
            ...
            Transform subFoodHolder = foodHolder.GetChild(0);
            Vector3 originalScale = foodHolder.localScale;
            float elapsedTime = 0f;
            bool isDrink = true;

            if (foodHolder.CompareTag("Beer") || subFoodHolder.CompareTag("Beer"))
            {
                isDrink = true;
